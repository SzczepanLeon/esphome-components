...
struct NumericFormula {
  NumericFormula(FormulaImplementation *f, SIUnit u)
      : formula_(f), siunit_(u) {}
...
struct NumericFormulaConstant : public NumericFormula {
  NumericFormulaConstant(FormulaImplementation *f, Unit u, double c)
      : NumericFormula(f, u), constant_(c) {}
...
struct NumericFormulaMeterField : public NumericFormula {
  NumericFormulaMeterField(FormulaImplementation *f, Unit u, std::string v,
                           Quantity q)
...
struct NumericFormulaDVEntryField : public NumericFormula {
  NumericFormulaDVEntryField(FormulaImplementation *f, Unit u,
                             DVEntryCounterType ct)
...
struct NumericFormulaPair : public NumericFormula {
  NumericFormulaPair(FormulaImplementation *f, SIUnit siu,
                     ...
                     std::unique_ptr<NumericFormula> &b, std::string name,
                     std::string op)
      : NumericFormula(f, siu), left_(std::move(a)), right_(std::move(b)),
        name_(name), op_(op) {}
...
struct FormulaImplementation : public Formula {
  ...
  // Pops the two top nodes of the formula builder stack and pushes an addition
  // on the formula builder stack. The target unit will be the supplied to unit.
  void doAddition(const SIUnit &to);
  // Pops the two top nodes of the formula builder stack and pushes a
  // subtraction on the formula builder stack. The target unit will be the
  // supplied to unit.
  void doSubtraction(const SIUnit &to);
  // Pops the two top nodes of the formula builder stack and pushes a
  // multiplication on the formula builder stack. The target unit will be
  // multiplication of the SI Units.
  void doMultiplication();
  // Pops the two top nodes of the formula builder stack and pushes a division
  // on the formula builder stack. The target unit will be first SIUnit divided
  // by the second SIUnit.
  void doDivision();
  // Pops the two top nodes of the formula builder stack and pushes an
  // exponentiation on the formula builder stack. The target unit will be first
  // SIUnit exponentiated.
  void doExponentiation();
  // Pops the single top node of the formula builder stack and pushes an
  // squareroot on the formula builder stack. The target unit will be SIUnit
  // square rooted.
  void doSquareRoot();
...
struct StringInterpolatorImplementation : public StringInterpolator {
  // Create a string interpolation from for example: "historic_{storage_counter
  // / - 12 counter}_value" Which for a dventry with storage 13 will "generate
  // historic_1_value"
  bool parse(Meter *m, const std::string &f);
