...
// I.e. you can have a total_m3 and a total_kwh even though they share the same
// "total" vname since they have two different quantities (Volume and Energy).
...
struct NumericField {
  ...
  NumericField() {}
  NumericField(Unit u, double v, FieldInfo *f)
      : unit(u), value(v), field_info(f) {}
  NumericField(Unit u, double v, FieldInfo *f, DVEntry &dve)
      : unit(u), value(v), field_info(f), dv_entry(dve) {}
};
...
protected:
  ...
  void addNumericFieldWithExtractor(
      std::string vname, // Name of value without unit, eg "total"
                         // "total_month{storagenr}"
      std::string help,  // Information about this field.
      PrintProperties print_properties, // Should this be printed by default in
                                        // fields,json and hr.
      Quantity vquantity,     // Value belongs to this quantity, this quantity
                              // determines the default unit.
      VifScaling vif_scaling, // How should any Vif value be scaled.
      DifSignedness dif_signedness, // Should we override the default signed
                                    // assumption for binary values?
      FieldMatcher matcher,
      Unit display_unit =
          Unit::Unknown,   // If specified use this unit for the json field
                           // instead instead of the default unit.
      double scale = 1.0); // A hard coded extra scale factor. Useful for
  ...
  void addNumericFieldWithCalculator(
      ...
      Quantity vquantity,  // Value belongs to this quantity, this quantity
                           // determines the default unit.
      std::string formula, // The formula can reference the other fields and +
                           // them together.
      Unit display_unit =
          Unit::Unknown); // If specified use this unit for the json field
                          // instead instead of the default unit.
  ...
  void addStringFieldWithExtractor(std::string vname, std::string help,
  ...
  void addStringField(std::string vname, std::string help,
                      PrintProperties print_properties);
  ...
  // Json fields include all values except timestamp_ut, timestamp_utc,
  // timestamp_lt since Json is assumed to be decoded by a program and the
  // current timestamp which is the same as timestamp_utc, can always be
  // decoded/recoded into local time or a unix timestamp.
...
protected:
  ...
  // This is the number of fields in the driver, not counting the used library
  // fields.
  size_t num_driver_fields_{};
  std::vector<std::string> field_names_;
  // Defaults to a setting specified in the driver. Can be overridden in the
  // meter file. There is also a global selected_fields that can be set on the
  // command line or in the conf file.
