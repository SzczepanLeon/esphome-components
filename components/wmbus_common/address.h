...
/**
    ...
    @ID_MFCT: Used when you have two meters with the same id but different
   manufacturers.
    @FULL: Used when you want to fully separate meter content on id.mft.v.t
    @NONE: Do not separate any meters! This might lead to telegrams overwriting
   each others state. Use this when no state is to be kept in the wmbusmeters
   object.
...
struct AddressExpression {
  // An address expression is used to select which telegrams to decode for a
  // driver. An address expression is also used to select a specific meter to
  // poll for data. Example address: 12345678 Or fully qualified:
  // 12345678.M=PII.T=1b.V=01 which means manufacturer triplet PII,
  // type/media=0x1b, version=0x01 Or wildcards in id: 12*.T=16 which matches
  // all cold water meters whose ids start with 12. Or negated tests:
  // 12345678.V!=66 which will decode all telegrams from 12345678 except those
  // where the version is 0x66. Or every telegram which is does not start with
  // 12 and is not from ABB: !12*.M!=ABB
  ...
  AddressExpression() {}
  AddressExpression(Address &a)
      : id(a.id), mfct(a.mfct), version(a.version), type(a.type) {}
  ...
  std::string str();
  static std::string
  concat(std::vector<AddressExpression> &address_expressions);
...
bool doesTelegramMatchExpressions(
    std::vector<Address> &addresses,
    std::vector<AddressExpression> &address_expressions, bool *used_wildcard);
