...
double NumericFormulaAddition::calculate(SIUnit to_siunit) {
  ...
  debug("(formula) ADD %g (%s) %g (%s) --> %g %s --> %g %s\n", l,
        left_->siunit().info().c_str(), r, right_->siunit().info().c_str(), v,
        v_siunit.info().c_str(), result, siunit().info().c_str());
...
double NumericFormulaMultiplication::calculate(SIUnit to_siunit) {
  ...
  debug("(formula) MUL %g (%s) %g (%s) --> %g --> %g %s\n", l,
        left_->siunit().info().c_str(), r, right_->siunit().info().c_str(), m,
...
double NumericFormulaDivision::calculate(SIUnit to_siunit) {
  ...
  debug("(formula) DIV %g (%s) %g (%s) --> %g --> %g %s\n", l,
        left_->siunit().info().c_str(), r, right_->siunit().info().c_str(), d,
        v, to_siunit.info().c_str());
...
double NumericFormulaSquareRoot::calculate(SIUnit to_siunit) {
  ...
  debug("(formula) SQRT %g (%s) --> %g --> %g %s\n", i,
        inner_->siunit().info().c_str(), s, v, to_siunit.info().c_str());
...
std::string Token::vals(const std::string &s) { return s.substr(start, len); }
...
size_t FormulaImplementation::findTime(size_t i) {
  ...
  if (i + 9 < formula_.length() && '\'' == formula_[i + 0] &&
      isdigit(formula_[i + 1]) && isdigit(formula_[i + 2]) &&
      ':' == formula_[i + 3] && isdigit(formula_[i + 4]) &&
      isdigit(formula_[i + 5]) && ':' == formula_[i + 6] &&
      isdigit(formula_[i + 7]) && isdigit(formula_[i + 8]) &&
      '\'' == formula_[i + 9]) {
    return 10;
  }

  if (i + 6 < formula_.length() && '\'' == formula_[i + 0] &&
      isdigit(formula_[i + 1]) && isdigit(formula_[i + 2]) &&
      ':' == formula_[i + 3] && isdigit(formula_[i + 4]) &&
      isdigit(formula_[i + 5]) && '\'' == formula_[i + 6]) {
    return 7;
...
size_t FormulaImplementation::findUnit(size_t i) {
  ...
  // All units start with a lower case a-z, followed by more letters and _
  // underscores.
  if (!is_letter(c))
    return 0;
...
size_t FormulaImplementation::findField(size_t i) {
  ...
  while (true) {
    if (i >= formula_.length())
      break;
    char c = formula_[i];
    // After first letter field names can contain more letters digits and
    // underscores.
...
size_t FormulaImplementation::parseOps(size_t i) {
  ...
  if (tok->type == TokenType::NUMBER) {
    if (next == NULL || next->type != TokenType::UNIT) {
      errors_.push_back(tostrprintf(
          "Constant number %s lacks a valid unit!\n%s",
          tok->vals(formula_).c_str(), tok->withMarker(formula_).c_str()));
      valid_ = false;
      return i;
    }
...
void FormulaImplementation::handleAddition(Token *tok) {
  ...
  bool ok =
      left_siunit.mathOpTo(MathOp::ADD, 0, 0, right_siunit, &to_siunit, NULL);
...
void FormulaImplementation::handleSubtraction(Token *tok) {
  ...
  bool ok =
      left_siunit.mathOpTo(MathOp::SUB, 0, 0, right_siunit, &v_siunit, NULL);
...
void FormulaImplementation::handleField(Token *field) {
  ...
  if (!ok) {
    errors_.push_back("Cannot extra a valid unit from field name \"" +
  ...
  if (dve != DVEntryCounterType::UNKNOWN) {
    debug("(formula) handle dventry field %s into %s %s\n", field_name.c_str(),
          vname.c_str(), unitToStringLowerCase(named_unit).c_str());
...
double FormulaImplementation::calculate(Unit to, DVEntry *dve, Meter *m) {
  ...
  if (op_stack_.size() != 1) {
    std::string t = tree();
    warning("Warning! Formula is not valid! Multiple ops on stack! Returning "
            "nan!\n%s\n",
            t.c_str());
    return std::nan("");
  }
...
std::string FormulaImplementation::str() { return topOp()->str(); }
...
std::string NumericFormulaConstant::tree() {
  ...
  return tostrprintf("<CONST %.17g %s[%s]%s> ", constant_,
                     unitToStringLowerCase(nearest).c_str(), sis.c_str(),
                     toString(q));
}
...
std::string NumericFormulaMeterField::tree() {
  if (formula()->meter() == NULL)
    return "<?" + vname_ + "?>";
  FieldInfo *fi = formula()->meter()->findFieldInfo(vname_, quantity_);
  return "<FIELD " + fi->vname() + "_" +
         unitToStringLowerCase(fi->displayUnit()) + "> ";
}

std::string NumericFormulaDVEntryField::str() { return toString(counter_); }
...
SIUnit &FormulaImplementation::siUnit() { return topOp()->siunit(); }
...
bool StringInterpolatorImplementation::parse(Meter *m, const std::string &f) {
  ...
  while (next_start_brace != std::string::npos) {
    ...
    std::string part =
        f.substr(prev_string_start, next_start_brace - prev_string_start);
    strings_.push_back(part);

    // Find the end of the formula.
    size_t next_end_brace = f.find('}', next_start_brace);
    if (next_end_brace == std::string::npos)
      return false; // Oups, missing closing }

    std::string formula =
        f.substr(next_start_brace + 1, next_end_brace - next_start_brace - 1);
