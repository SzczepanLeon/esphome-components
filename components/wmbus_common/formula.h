...
struct Formula {
  ...
  // If a dventry is supplied, then the values storage_counter, tariff_counter,
  // subunit_counter are available. If a meter is supplied it overrides the
  // meter supplied when parsing.
...
struct StringInterpolator {
  /**
     parse: Parse a field name like: "historic_{storage_counter / - 12
     counter}_value"
     @f: String to parse and find {...} subformulas inside.

     Parse and prepare it for being applied to an actual dve.
     E.g. for a dventry with storage 13 the formula above will generate
     "historic_1_value"
  */
  virtual bool parse(Meter *m, const std::string &f) = 0;
  /**
     apply: Insert inline formulas in field names like "total_{subunit_nr}"
     insert the result of the formula.
     @dve: A pointer to a telegram entry to use (ie containing
     storagenr/tariffnr/subunitnr etc.) Can be NULL.
