...
splitSequenceOfAddressExpressionsAtCommas(const std::string &mes);
...
bool doesAddressMatchExpressions(
    Address &address, std::vector<AddressExpression> &address_expressions,
    bool *used_wildcard, bool *filtered_out, bool *required_found,
...
bool isValidMatchExpression(const std::string &s, bool *has_wildcard) {
  ...
  while (me.length() > 0 && ((me.front() >= '0' && me.front() <= '9') ||
...
bool AddressExpression::match(const std::string &i, uint16_t m, uchar v,
                              uchar t) {
...
std::string
AddressExpression::concat(std::vector<AddressExpression> &address_expressions) {
  std::string s;
...
void Address::decodeMfctFirst(const std::vector<uchar>::iterator &pos) {
  mfct = *(pos + 1) << 8 | *(pos + 0);
  id = tostrprintf("%02x%02x%02x%02x", *(pos + 5), *(pos + 4), *(pos + 3),
                   *(pos + 2));
  version = *(pos + 6);
  type = *(pos + 7);
}
...
void Address::decodeIdFirst(const std::vector<uchar>::iterator &pos) {
  id = tostrprintf("%02x%02x%02x%02x", *(pos + 3), *(pos + 2), *(pos + 1),
                   *(pos + 0));
  mfct = *(pos + 5) << 8 | *(pos + 4);
  version = *(pos + 6);
  type = *(pos + 7);
...
bool doesTelegramMatchExpressions(
  ...
  bool required_failed =
      true; // Init to fail, set to true if R is satistifed anywhere.

  for (Address &a : addresses) {
    if (doesAddressMatchExpressions(a, address_expressions, used_wildcard,
                                    &filtered_out, &required_found,
                                    &required_failed)) {
      match = true;
    }
  ...
  // If any expression triggered a filter out, then the whole telegram does not
  // match.
  if (filtered_out)
    match = false;
...
bool doesAddressMatchExpressions(
  ...
  for (AddressExpression &ae : address_expressions) {
    ...
    // We currently assume that only a single expression is required, the last
    // one!
...
bool AddressExpression::operator==(const AddressExpression &ae) const {
  return id == ae.id && has_wildcard == ae.has_wildcard &&
         mbus_primary == ae.mbus_primary && mfct == ae.mfct &&
         version == ae.version && type == ae.type &&
